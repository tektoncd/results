...
func TestRecorder_RecordStorageLatency(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "successful TaskRun storage with pipeline task label",
			runType: TaskRunType,
			object: &pipelinev1.TaskRun{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "taskrun-pipeline-1",
					Namespace: "ns",
					Labels: map[string]string{
						"tekton.dev/pipelineTask": "first-add",
						"tekton.dev/pipeline":     "sum-pipeline",
						"tekton.dev/pipelineRun":  "sum-pipeline-run",
					},
				},
				Spec: pipelinev1.TaskRunSpec{
					TaskRef: &pipelinev1.TaskRef{Name: "add-task"},
				},
				Status: pipelinev1.TaskRunStatus{
					TaskRunStatusFields: pipelinev1.TaskRunStatusFields{
						CompletionTime: &completionTime,
			...
			expectedLatencyTags: map[string]string{
				...
				"task":      "first-add", // Should use the pipeline task name, not the template name
				"namespace": "ns",
			},
			expectedLatencyValue: 60.0, // 1 minute
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Record the metric
			err = r.RecordStorageLatency(context.Background(), tt.runType, tt.object)
