...
const (
	logsServiceName = "tekton.results.v1alpha3.Logs"
)
...
func WithContext(ctx context.Context, conn *grpc.ClientConn) (context.Context, error) {
	...
	for _, service := range response.GetListServicesResponse().GetService() {
		if service.Name == logsServiceName {
			return context.WithValue(ctx, Key{}, v1alpha3pb.NewLogsClient(conn)), nil
		}
	}
...
func Get(ctx context.Context) v1alpha3pb.LogsClient {
	...
	return untyped.(v1alpha3pb.LogsClient)
}
