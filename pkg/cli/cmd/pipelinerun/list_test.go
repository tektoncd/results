...
func TestBuildFilterString(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: "data_type==\"PIPELINE_RUN\" AND data.metadata.namespace==\"test-ns\"",
		},
		{
			name: "with label",
			opts: &listOptions{
		...
		},
		{
			...
			expected: "data_type==\"PIPELINE_RUN\" AND data.metadata.namespace==\"test-ns\" AND data.metadata.labels.key==\"value\"",
		},
	}
...
func buildFilterString(opts *listOptions) string {
	...
	if opts.Namespace != "" {
		filter += fmt.Sprintf(" AND data.metadata.namespace==\"%s\"", opts.Namespace)
	}
	if opts.Label != "" {
