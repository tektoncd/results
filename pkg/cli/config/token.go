...
// usesDynamicToken reports whether the given rest.Config authenticates with a
// credential that is produced at request time rather than a static value.
//
// Such credentials - exec credential plugins (e.g. `oc get-token` for OpenShift
// external OIDC, `aws eks get-token`, `gke-gcloud-auth-plugin`), legacy
// auth-providers (oidc/gcp/azure), and token files - yield short-lived tokens
// that must be re-resolved on every request. Persisting them (as `config set`
// does when a prompt default is accepted) pins a token that expires and breaks
// future requests.
//
// A static token is present when BearerToken is set; any of the dynamic
// mechanisms is in play otherwise. This is a cheap check that never invokes a
// plugin or touches the network.
func usesDynamicToken(rc *rest.Config) bool {
	return rc != nil && rc.BearerToken == "" &&
		(rc.ExecProvider != nil || rc.AuthProvider != nil || rc.BearerTokenFile != "")
}
