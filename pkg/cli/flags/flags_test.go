...
func TestInitParamsWithEmptyFlags(t *testing.T) {
	...
	// Check that the Params object has empty values for optional flags
	if p.Namespace() != "" {
		t.Errorf("Expected Namespace to be empty, got '%s'", p.Namespace())
	}
	if p.Host() != "" {
		t.Errorf("Expected Host to be empty, got '%s'", p.Host())
	}
	if p.Token() != "" {
		t.Errorf("Expected Token to be empty, got '%s'", p.Token())
	}
	if p.APIPath() != "" {
		t.Errorf("Expected APIPath to be empty, got '%s'", p.APIPath())
	}
	if p.SkipTLSVerify() {
		t.Errorf("Expected SkipTLSVerify to be false, got %v", p.SkipTLSVerify())
	}
}
