...
func GetResultsOptions(cmd *cobra.Command) ResultsOptions {
	kcPath, _ := cmd.PersistentFlags().GetString(kubeConfig)
	kubeContext, _ := cmd.PersistentFlags().GetString(context)
	ns, _ := cmd.PersistentFlags().GetString(namespace)
	h, _ := cmd.PersistentFlags().GetString(host)
	t, _ := cmd.PersistentFlags().GetString(token)
	ap, _ := cmd.PersistentFlags().GetString(apiPath)
	skipTLSVerify, _ := cmd.PersistentFlags().GetBool(insecureSkipTLSVerify)
...
func InitParams(p common.Params, cmd *cobra.Command) error {
	...
	kubeContext, err := cmd.PersistentFlags().GetString(context)
	if err != nil {
		return err
	}
	...
	t, err := cmd.PersistentFlags().GetString(token)
	if err != nil {
		return err
	}
	if t != "" {
	...
	ap, err := cmd.PersistentFlags().GetString(apiPath)
	if err != nil {
		return err
	}
	if ap != "" {
	...
	skipTLSVerify, err := cmd.PersistentFlags().GetBool(insecureSkipTLSVerify)
	if err != nil {
		return err
	}
