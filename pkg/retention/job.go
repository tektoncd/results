...
func (a *Agent) job() {
	a.Logger.Infof("retention job started at: %s", time.Now().String())
	fmt.Println(-int(a.MaxRetention))
	fmt.Println(time.Now().Add(-a.MaxRetention))

	q := a.db.Unscoped().
		Where("updated_time < ?", time.Now().Add(-a.MaxRetention)).
		Delete(&db.Record{})
