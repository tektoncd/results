...
| `watcher_customrun_delete_duration_seconds_[bucket, sum, count]`   | The duration from CustomRun completion to deletion                         | Histogram                  | `*pipeline`=&lt;pipeline_name&gt; <br> `*customrun`=&lt;customrun_name&gt; <br> `status`=&lt;status&gt; <br> `namespace`=&lt;namespace&gt;                                                 | experimental |
| `watcher_pipelinerun_delete_count_total`                           | The total count of deleted PipelineRun                                     | Counter                    | `status`=&lt;status&gt; <br> `namespace`=&lt;pipelinerun-namespace&gt;                                                                                                                     | experimental |
| `watcher_taskrun_delete_count_total`                               | The total count of deleted TaskRun                                         | Counter                    | `status`=&lt;status&gt; <br> `namespace`=&lt;pipelinerun-namespace&gt;                                                                                                                     | experimental |
| `watcher_customrun_delete_count_total`                             | The total count of deleted CustomRuns                                      | Counter                    | `*pipeline`=&lt;pipeline_name&gt; <br> `*customrun`=&lt;customrun_name&gt; <br> `status`=&lt;status&gt; <br> `namespace`=&lt;namespace&gt;                                                 | experimental |
| `watcher_run_storage_latency_seconds_[bucket, sum, count]`         | The duration between run completion and successful storage                 | Histogram                  | `kind`=&lt;pipelinerun\|taskrun\|customrun&gt; <br> `namespace`=&lt;run_namespace&gt;                                                                                                      | experimental |
| `watcher_runs_not_stored_count_total`                              | Total number of runs which were deleted without being stored               | Counter                    | `kind`=&lt;pipelinerun\|taskrun\|customrun&gt; <br> `namespace`=&lt;run_namespace&gt;                                                                                                      | experimental |

**Notes**:
- Counter metrics have a `_total` suffix appended automatically by the OpenTelemetry Prometheus exporter, following standard Prometheus naming conventions.
...
- The `watcher_runs_not_stored_count_total` metric may be inflated in some rare cases as it can be incremented multiple times during reconciliation when the controller attempts to store the run again or when deletion doesn't complete immediately.
- CustomRun metrics granularity is controlled by `metrics.taskrun.level`, since CustomRuns are task-level resources. There is no separate `metrics.customrun.level` setting.
...
**Note:** The `metrics.taskrun.level` setting also controls the granularity of CustomRun metrics (`watcher_customrun_delete_*`). When set to `task`, the `customrun` label is included; when set to `namespace`, it is omitted.
