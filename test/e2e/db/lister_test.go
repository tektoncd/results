...
func TestLister_OrderByCreateTimeDesc(t *testing.T) {
	...
	if got := len(resp.Results); got != 6 {
		t.Fatalf("want 6 results, got %d", got)
	}
...
// It verifies status filtering, descending create_time order across pages,
// no duplicate names, and the expected total count.
func TestLister_PaginationWithFilterAndOrder(t *testing.T) {
	...
	for page := 0; ; page++ {
		...
		for _, r := range resp.Results {
			...
			if seenNames[r.Name] {
				t.Errorf("duplicate result across pages: %s", r.Name)
			}
			seenNames[r.Name] = true

			ct := r.CreateTime.AsTime()
			if totalCount > 0 && ct.After(prevCreateTime) {
				t.Errorf("result %s create_time (%v) is after previous (%v); want DESC order across pages",
					r.Name, ct, prevCreateTime)
			}
			prevCreateTime = ct
			totalCount++
		}
	...
	if totalCount != 12 {
		t.Errorf("want 12 SUCCESS results, got %d", totalCount)
	}
}
