...
**CI path (`e2e.sh`):** The script constructs a single `DB_URL` and passes it
to `go test`. The individual `POSTGRES_*` variables below are **not** used
by the script.

**Standalone / manual runs:** When `DB_URL` is not set, `db_test.go` falls
back to the individual `POSTGRES_*` variables.

| Variable | Default | Description |
|---|---|---|
| `DB_URL` | (none) | Full Postgres DSN. When set, takes precedence over all `POSTGRES_*` vars. The `e2e.sh` script constructs this automatically. |
| `PG_E2E_PORT` | `15432` | Local port for `kubectl port-forward` in `e2e.sh`. Override to avoid collisions in parallel runs. |
| `POSTGRES_HOST` | `localhost` | Standalone fallback: Postgres host (via port-forward) |
| `POSTGRES_PORT` | `15432` | Standalone fallback: local forwarded port |
| `POSTGRES_USER` | `postgres` | Standalone fallback: DB user |
| `POSTGRES_PASSWORD` | (none) | Standalone fallback: DB password (from cluster secret) |
| `POSTGRES_DB` | `tekton-results` | Standalone fallback: database name (the live API database) |
| `POSTGRES_SSLMODE` | `disable` | Standalone fallback: SSL mode |
